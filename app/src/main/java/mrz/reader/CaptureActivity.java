...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback,
    ...
    public void onCreate(Bundle icicle) {
        ...
        setContentView(R.layout.activiy_capture_mrz);
...
//            });
        ...
        translationView = findViewById(R.id.translation_text_view);
    ...
    boolean handleOcrDecode(OcrResult ocrResult) {
        ...
        ImageView bitmapImageView = findViewById(R.id.image_view);
        lastBitmap = ocrResult.getBitmap();
        if (lastBitmap == null) {
            bitmapImageView.setImageBitmap(BitmapFactory.decodeResource(getResources(),
                    R.drawable.logocapture));
        } else {
            bitmapImageView.setImageBitmap(lastBitmap);
        }

        // Display the recognized text
        TextView sourceLanguageTextView = findViewById(R.id.source_language_text_view);
        sourceLanguageTextView.setText(sourceLanguageReadable);
        TextView ocrResultTextView = findViewById(R.id.ocr_result_text_view);
        ocrResultTextView.setText(ocrResult.getText());
        // Crudely scale betweeen 22 and 32 -- bigger font for shorter text
        int scaledSize = Math.max(22, 32 - ocrResult.getText().length() / 4);
        ocrResultTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, scaledSize);

        TextView translationLanguageLabelTextView = findViewById(R.id.translation_language_label_text_view);
        TextView translationLanguageTextView = findViewById(R.id.translation_language_text_view);
...
//                        Toast.makeText(this, mrzInfo.toString(), Toast.LENGTH_LONG).show();
                        Log.wtf(TAG, mrzInfo.toString());
                        Intent returnIntent = new Intent();
                        returnIntent.putExtra(MRZ_RESULT, result);
    ...
    private boolean checkFirstLaunch() {
        try {
            ...
            if (currentVersion > lastVersion) {
                ...
                prefs.edit().putInt(PreferencesActivity.KEY_HELP_VERSION_SHOWN, currentVersion).apply();
